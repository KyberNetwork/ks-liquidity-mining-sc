...
contract KSElasticLMV2 is IKSElasticLMV2, KSAdmin, ReentrancyGuard {
    ...
    function updateTokenCode(
        bytes memory _farmingTokenCreationCode
    ) external isAdmin {
    ...
    function addPhase(
        uint256 fId,
        PhaseInput calldata phaseInput
    ) external isOperator {
        ...
        for (uint256 i; i < length; ) {
            ...
            if (
                ...
                phaseInput.rewards[i].rewardToken
    ...
    function forceClosePhase(uint256 fId) external isOperator {
        ...
        if (block.timestamp > farms[fId].lastTouchedTime)
            _updateFarmSumRewardPerLiquidity(fId);
    ...
    function removeRange(uint256 fId, uint256 rangeId) external isOperator {
        if (fId >= farmCount) revert InvalidFarm();
        if (
            rangeId >= farms[fId].ranges.length ||
            farms[fId].ranges[rangeId].isRemoved
    ...
    function deposit(
        ...
        uint256[]
            memory curSumRewardPerLiquidity = _updateFarmSumRewardPerLiquidity(
                fId
            );
        ...
        for (uint256 i; i < nftIds.length; ) {
            ...
            for (uint256 j; j < rewardLength; ) {
                stakes[nftIds[i]].lastSumRewardPerLiquidity.push(
                    curSumRewardPerLiquidity[j]
                );
        ...
        if (farmingToken != address(0))
            _mintFarmingToken(farmingToken, receiver, totalLiquidity);
    ...
    function withdraw(
        ...
        for (uint256 i; i < length; ) {
            ...
            if (!depositNFTs[msg.sender].remove(nftIds[i]))
                revert FailToRemove();
    ...
    function addLiquidity(
        ...
        for (uint256 i; i < length; ) {
            ...
            if (newLiq > curLiq)
                _updateLiquidity(fId, nftIds[i], newLiq, msg.sender);
    ...
    function removeLiquidity(
        ...
        uint256 newLiq = posLiq *
            farms[fId].ranges[stakes[nftId].rangeId].weight;
    ...
    function withdrawEmergency(uint256[] calldata nftIds) external override {
        ...
        for (uint256 i; i < length; ) {
            ...
            if (!emergencyEnabled) {
                ...
                if (!depositNFTs[stake.owner].remove(nftId))
                    revert FailToRemove();
    ...
    function _updateLiquidity(
        ...
        uint256[]
            memory curSumRewardPerLiquidities = _updateFarmSumRewardPerLiquidity(
                fId
            );
        uint256 curLiq = stakes[nftId].liquidity;
    ...
    function _updateFarmSumRewardPerLiquidity(
        ...
        for (uint256 i; i < length; ) {
            uint256 preSumRewardPerLiquidity = farms[fId].sumRewardPerLiquidity[
                i
            ];
            ...
            if (curSumRewardPerLiquidity[i] != preSumRewardPerLiquidity)
                farms[fId].sumRewardPerLiquidity[i] = curSumRewardPerLiquidity[
                    i
                ];
        ...
        if (block.timestamp > endTime && !isSettled)
    ...
    function _removeLiquidity(
        ...
        IBasePositionManager.RemoveLiquidityParams
            memory removeLiq = IBasePositionManager.RemoveLiquidityParams({
                tokenId: nftId,
                liquidity: liquidity,
                amount0Min: 0,
                amount1Min: 0,
    ...
    function _claimFee(uint256 nftId, uint256 deadline, bool syncFee) internal {
        ...
        IBasePositionManager.BurnRTokenParams
            memory burnRToken = IBasePositionManager.BurnRTokenParams({
                tokenId: nftId,
                amount0Min: 0,
                amount1Min: 0,
                deadline: deadline
    ...
    function _calcSumRewardPerLiquidity(
        ...
        if (
            block.timestamp > lastTouchedTime &&
            totalLiquidity != 0 &&
            !isSettled
        ) {
            uint256 deltaSumRewardPerLiquidity = LMMath
                .calcSumRewardPerLiquidity(
                    rewardAmount,
                    startTime,
                    endTime,
    ...
    function _isRangeValid(RangeInput memory range) internal pure {
        if (range.tickLower > range.tickUpper || range.weight == 0)
            revert InvalidRange();
    }
    ...
    function _isAddLiquidityValid(uint256 fId, uint256 rangeId) internal view {
        ...
        if (
            farms[fId].phase.endTime < block.timestamp ||
            farms[fId].phase.isSettled
        ) revert PhaseSettled();
