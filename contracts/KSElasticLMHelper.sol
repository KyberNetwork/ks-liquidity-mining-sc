...
contract KSElasticLMHelper is IKSElasticLMHelper, KSAdmin {
    ...
    function checkPool(
        ...
        IBasePositionManager.Position memory pData = _getPositionFromNFT(
            nftContract,
            nftId
        );
        return
            IBasePositionManager(nftContract).addressToPoolId(pAddress) ==
            pData.poolId;
    }
    ...
    function getActiveTime(
        ...
        return
            IPoolStorage(pAddr).getSecondsPerLiquidityInside(
                pData.tickLower,
                pData.tickUpper
            );
    }
    ...
    function getPair(
        ...
        (
            ,
            IBasePositionManager.PoolInfo memory poolInfo
        ) = IBasePositionManager(nftContract).positions(nftId);
    ...
    function getEligibleRanges(
        ...
        (
            address poolAddr,
            IELMV2.RangeInfo[] memory rangesInfo,
            ,
            ,
            ,
            ,
    ...
    function getUserInfo(
        ...
        for (uint256 i = 0; i < listNFTs.length; ++i) {
            (, uint256 fId, uint256 rId, uint256 sLiq, , ) = farm.getStake(
                listNFTs[i]
            );
            ...
            result[i].currentUnclaimedRewards = getCurrentUnclaimedReward(
                farm,
                listNFTs[i]
            );
        }
    ...
    function checkPosition(
        ...
        view
        override
        returns (bool isInvalid, uint128[] memory liquidities)
    {
        uint256 length = nftIds.length;
        liquidities = new uint128[](length);
        uint256 poolId = IBasePositionManager(nftContract).addressToPoolId(
            pAddress
        );
    ...
    function _getPositionFromNFT(
        ...
        (IBasePositionManager.Position memory pData, ) = IBasePositionManager(
            nftContract
        ).positions(nftId);
        return pData;
    }
    ...
    function _getFeePool(
        ...
        IBasePositionManager.Position memory position = _getPositionFromNFT(
            nftContract,
            nftId
        );
        (, , uint256 lowerValue, ) = IPoolStorage(poolAddress).ticks(
            position.tickLower
        );
        (, , uint256 upperValue, ) = IPoolStorage(poolAddress).ticks(
            position.tickUpper
        );
        (, int24 currentTick, , ) = IPoolStorage(poolAddress).getPoolState();
        uint256 feeGrowthGlobal = IPoolStorage(poolAddress)
            .getFeeGrowthGlobal();

        {
            ...
            ) = IPoolStorage(poolAddress).getLiquidityState();
            ...
            if (rMintQty != 0) {
                ...
                (, uint24 governmentFeeUnits) = IPoolStorage(poolAddress)
                    .factory()
                    .feeConfiguration();
                unchecked {
                    if (governmentFeeUnits != 0) {
                        uint256 rGovtQty = (rMintQty * governmentFeeUnits) /
                            C.FEE_UNITS;
                        rMintQty -= rGovtQty;
                    }
                    feeGrowthGlobal += FullMath.mulDivFloor(
                        rMintQty,
                        C.TWO_POW_96,
                        baseL
                    );
