...
contract MockNftManagerV4 is ERC721 {
    ...
    function setMockLiquidity(
        uint256 tokenId,
        uint128 _mockLiquidity
    ) external {
    ...
    function transferAllTokens(
        ...
        MockToken(token).transfer(
            recipient,
            MockToken(token).balanceOf(address(this))
        );
    }

    function setMockTick(
        uint256 tokenId,
        int24 tickLower,
        int24 tickUpper
    ) public {
    ...
    function setMockFeeGrowthInsideLast(
        uint256 tokenId,
        uint256 _mockFeeGrowthInsideLast
    ) external {
