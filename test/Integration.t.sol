...
contract Integration is FoundryHelper {
    ...
    function test_In_01() public {
        ...
        rw[0] = IELM3.RewardInput({
            rewardToken: address(token),
            rewardAmount: 140 ether
        });
        rw[1] = IELM3.RewardInput({
            rewardToken: ETH_ADDRESS,
            rewardAmount: 310 ether
        });

        IELM3.PhaseInput memory p = IELM3.PhaseInput({
            startTime: st,
            endTime: et,
            rewards: rw
        });
    ...
    function test_In_03() public {
        ...
        IELM3.RangeInput memory newRange = IELM3.RangeInput({
            tickLower: -8,
            tickUpper: -3,
            weight: 2
        });
    ...
    function test_In_08() public {
        ...
        changePrank(deployer);
        p = IELM3.PhaseInput({
            startTime: curr + 1 days,
            endTime: curr + 31 days,
            rewards: rw
        });
        farm.addPhase(fId, p);
        token.transfer(address(farm), 100 ether);
    ...
    function test_In_17() public {
        ...
        assertEq(
            IKyberSwapFarmingToken(fToken).balanceOf(address(user1)),
            70 ether
        ); // 30 index 0 + 20*2 index 1
        ...
        changePrank(deployer);
        IKyberSwapFarmingToken(fToken).transferFrom(
            address(user1),
            address(user2),
            20 ether
        );
        ...
        assertEq(
            IKyberSwapFarmingToken(fToken).balanceOf(address(user2)),
            30 ether
        );
    }
    ...
    function test_In_18() public {
        ...
        uint256[] memory indexesValid = helperSC.getEligibleRanges(
            farm,
            fId,
            nft0
        );
        ...
        uint256[] memory indexesValid2 = helperSC.getEligibleRanges(
            farm,
            fId,
            nft1
        );
    ...
    function test_In_19() public {
        ...
        farm.deposit(fId, 0, listNFT, user1);
        vm.warp(st + 10 days);
        uint256[] memory currentUnclaimedRewards = helperSC
            .getCurrentUnclaimedReward(farm, nft0);
        assertEq(currentUnclaimedRewards.length, 1);
        assertEq(currentUnclaimedRewards[0], 60000000000000000000);
        uint256[] memory currentUnclaimedRewards1 = helperSC
            .getCurrentUnclaimedReward(farm, nft1);
        ...
        vm.warp(st + 20 days);
        currentUnclaimedRewards = helperSC.getCurrentUnclaimedReward(
            farm,
            nft0
        );
        assertEq(currentUnclaimedRewards[0], 202105263157894736842);
        currentUnclaimedRewards1 = helperSC.getCurrentUnclaimedReward(
            farm,
            nft1
        );
        ...
        rw[0] = IELM3.RewardInput({
            rewardToken: address(token),
            rewardAmount: 60 ether
        });
        p = IELM3.PhaseInput({
            startTime: et + 30 days,
            endTime: et + 60 days,
            rewards: rw
        });
        farm.addPhase(fId, p);
    ...
    function test_In_20_deposit_list_nfts() public {
        ...
        (, , , stakedLiq, lastSumRewardPerLiquidity, rewardUnclaimed) = farm
            .getStake(nft0);
    ...
    function test_In_23_claimReward_list_nfts_with_sameId() public {
        ...
        assertApproxEqAbs(
            balanceETHAfter - balanceETHBefore,
            5555555555555555555,
            10
        );
    }
